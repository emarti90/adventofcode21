...
# Read Input Data
df = pd.read_csv("Day4.txt", sep = "\n", header = None)

# Formatting data
numbers = str(df.iloc[0][0]).split(',')

data = df.drop(labels=0, axis=0)
data = [x.split(' ') for x in data[0]]
data = [list(filter(lambda a: a!='',x)) for x in data]
boards = pd.DataFrame(data)
boards[5] = [str(x//5) for x in range(len(data))]
...
# Calculate result
board = boards[boards.columns[0:5]].iloc[min(rows):max(rows)+1]
